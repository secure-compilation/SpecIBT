...
Proof.
  intros. eapply seq_small_step_obs_type in X; [|now apply X0].
  destruct os1; subst; [now auto|].
  destruct o.
  2, 3 : do 2 (destruct X); destruct H; subst; reflexivity.
  all : now destruct X; subst.
Qed.
...
Proof.
  intros. induction X; repeat econstructor; eauto.
Qed.

(* this way was too difficult for Rocq to parse 
...
Lemma multi_spec_seq : forall p c1 c2 cm st ast b stm astm bm ds os,
  ...
  (Datatypes.sum ({st':_ & {ast':_ & {b':_ & {ds1:_ & {ds2:_ & {os1:_ & {os2:_ &
    (Datatypes.prod (os = os1 ++ os2)
      (Datatypes.prod (ds = ds1 ++ ds2)
        (Datatypes.prod (p |- <((c1, st, ast, b))> -->*_ds1^^os1 <((skip, st', ast', b'))>)
                        (p |- <((c2, st', ast', b'))> -->*_ds2^^os2 <((cm, stm, astm, bm))>)
        )))}}}}}}})
    ({c':_ & (Datatypes.prod (cm = <{{ c'; c2 }}>)
              (p |- <((c1, st, ast, b))> -->*_ds^^os <((c', stm, astm, bm))>))})).
Proof.
  ...
  + edestruct IHX; [reflexivity|..].
    - do 8 destruct s as [? s]. destruct s, p0. subst. clear IHX.
...
Lemma multi_seq_seq : forall p c1 c2 cm st ast stm astm os,
  ...
  (Datatypes.sum 
    ({st':_ & {ast':_ & {os1:_ & {os2:_ & 
      (Datatypes.prod (os = os1 ++ os2) 
        (Datatypes.prod (p |- <((c1, st, ast))> -->*^os1 <((skip, st', ast'))>)
                        (p |- <((c2, st', ast'))> -->*^os2 <((cm, stm, astm))>)
        ))}}}})
    ({c':_ & 
      (Datatypes.prod (cm = <{{ c'; c2 }}>)
...
Proof.
  ...
  invert s.
  + edestruct IHX; [reflexivity|..].
    - do 6 destruct s as [? s]. subst. clear IHX.
      left. rewrite !app_assoc. repeat eexists; [econstructor|]; eassumption.
    - do 2 destruct s as [? s]. subst. clear IHX.
      right. repeat eexists. econstructor; eassumption.
  + left. repeat eexists; [constructor|eassumption].
...
Lemma multi_spec_seq_assoc p c1 c2 c3 st ast b c' st' ast' b' ds os :
  ...
  ({c'':_ & (Datatypes.prod (p |- <((c1; c2; c3, st, ast, b))> -->*_ds^^os <((c'', st', ast', b'))>) 
      (c' = <{{ skip }}> -> c'' = <{{ skip }}>))
  }).
Proof.
  intros. apply multi_spec_seq in X. destruct X.
  + do 10 destruct s as [? s]. subst. apply multi_spec_seq in m. destruct m.
    - do 10 destruct s0 as [? s0]. subst. exists c'. split; [|tauto].
      rewrite <- !app_assoc. eapply multi_spec_combined_executions; [apply multi_spec_add_snd_com, m|].
      eapply multi_spec_trans_nil_l; [apply SpecSM_Seq_Skip|]. eapply multi_spec_combined_executions; [apply multi_spec_add_snd_com, s0|].
      eapply multi_spec_trans_nil_l; [apply SpecSM_Seq_Skip|apply s].
    - destruct s0 as (c''&abs&_). discriminate abs.
  + destruct s as (c''&->&s). apply multi_spec_seq in s. destruct s.
    - do 10 destruct s as [? s]. subst. exists <{{ c''; c3 }}>.
      split; [|discriminate]. eapply multi_spec_combined_executions; [apply multi_spec_add_snd_com, m|].
      eapply multi_spec_trans_nil_l; [apply SpecSM_Seq_Skip|]. apply multi_spec_add_snd_com, s.
    - destruct s as (c'&->&s). exists <{{ c'; c2; c3 }}>. split; [|discriminate].
      apply multi_spec_add_snd_com, s.
Qed.

(** * Definition of Relative Secure *)

Definition seq_same_obs p c st1 st2 ast1 ast2 : Type :=
...
Definition spec_same_obs p c st1 st2 ast1 ast2 : Type :=
...
(*  old definition:
   Definition relative_secure (p:prog) (trans : com -> com)
    (c:com) (st1 st2 : state) (ast1 ast2 : astate): Type :=
...
Definition nonempty_arrs (ast : astate) : Type :=
...
Definition ideal_same_obs p c st1 st2 ast1 ast2 : Type :=
  forall ds stt1 stt2 astt1 astt2 bt1 bt2 os1 os2 c1 c2,
...
Proof.
  intros p c st ast ds ct stt astt bt os Heval. remember true as b eqn:Eqb.
  induction Heval; subst; eauto. apply ideal_eval_small_step_spec_bit_monotonic in i; subst.
  auto.
Qed.
...
Proof.
  intros. remember false as b. rewrite Heqb in X at 2. remember false as b'.
  rewrite Heqb' in Heqb.
  revert Heqb Heqb' d H.
  induction X; intros; (try discriminate); subst; try (now inversion H).
  + apply IHX; tauto.
Qed.
...
Proof.
  intros. remember false as b. rewrite Heqb in X at 2. remember false as b'.
  rewrite Heqb' in Heqb. revert Heqb Heqb' d H.
  induction X; intros; subst.
  + now apply in_nil in H.
  + destruct b'. { now apply multi_ideal_spec_bit_monotonic in X. }
...
Lemma ideal_eval_small_step_spec_needs_force : forall p c st ast ds ct stt astt os,
  ...
  (Datatypes.sum (ds = [DForce]) ({j:_ & (ds = [DForceCall j])})).
Proof.
...
Lemma multi_ideal_spec_needs_force : forall p c st ast ds ct stt astt os,
  ...
  (Datatypes.sum (In DForce ds) ({j:_ & (In (DForceCall j) ds)})).
Proof.
  intros p c st ast ds ct stt astt os Hev.
  remember false as b eqn:Eqb; remember true as bt eqn:Eqbt.
  induction Hev; subst; simpl in *; try discriminate.
  destruct b' eqn:Eqb'.
  - apply ideal_eval_small_step_spec_needs_force in i.
    destruct i as [ H1 | H2 ]; subst; simpl; try (left; tauto); 
...
Proof.
  intros p c st ast b ds ct stt astt bt os Hev. induction Hev; simpl; auto.
  do 2 rewrite length_app. apply ideal_eval_small_step_obs_length in i.
  auto.
Qed.
...
Proof.
  ...
  - apply in_app_or in Hin as [Hin | Hin].
    + destruct b eqn:Eqb.
      * apply ideal_eval_small_step_spec_bit_monotonic in i. discriminate.
  ...
  - assert (L1: forall d, In d ds1 -> d = DStep).
    ...
    + apply ideal_eval_small_step_spec_needs_force in i.
      ...
      * destruct HDFC as [j HDFC]. subst. simpl in L1.
        ...
        discriminate L1.
    + apply ideal_eval_small_step_no_spec in i; auto.
...
Proof.
  intros.
  induction X; try now (constructor; rewrite ?orb_true_r, ?andb_false_r).
Qed. 

Lemma seq_small_step_if_total : forall p c be ct cf st ast,
  c = <{{if be then ct else cf end}}> ->
  {c':_ & {stt:_ & {astt:_ & {os:_ & 
    (Datatypes.prod (p |- <((c, st, ast))> -->^os <((c', stt, astt))>) (os <> []))}}}}.
Proof.
...
Proof.
  ...
  assert (L1: {c':_ & {stt:_ & {astt:_ & {os:_ & 
                (Datatypes.prod (p |- <((c, st1, ast1))> -->^os <((c', stt, astt))>) (os <> []))
          }}}}). { eapply seq_small_step_if_total; eauto. }   
  assert (L2: {c':_ & {stt:_ & {astt:_ & {os:_ & 
                (Datatypes.prod (p |- <((c, st2, ast2))> -->^os <((c', stt, astt))>) (os <> []))
          }}}}). { eapply seq_small_step_if_total; eauto. }
  ...
  - inversion Hev2; subst; clear Hev2.
    + destruct Hneq2; auto.
    + inversion X1; subst; clear X1. inversion X; subst; clear X.
...
Proof.
  ...
  - apply seq_same_obs_com_seq in Hobs. destruct IHHev1 with (j:=j) (st2:=st2) 
    (ast2:=ast2) (stt2:=stt2) (astt2:=astt2) (os2:=os2); auto. inversion X; subst; auto.
    discriminate.
  - inversion X; auto.
Qed.
...
Lemma ideal_unused_overwrite_one_step (p:prog) : forall st ast b ds c c' st' ast' b' os X n,
  ...
  unused X c ->
  p |- <((c, st, ast, b))> -->i_ds^^os <((c', st', ast', b'))> ->
  (Datatypes.prod (p |- <((c, X !-> n; st, ast, b))> -->i_ds^^os <((c', X !-> n; st', ast', b'))>)
                  (unused X c')).
Proof.
  intros. remember (unused_prog X p) as unused_p. induction X0.
  - invert H0. repeat econstructor. rewrite t_update_permute; [constructor|assumption].
    now apply aeval_unused_update.
  - invert H0. eapply IHX0 in H1. destruct H1.
    repeat econstructor; assumption.
  - split; [|now invert H0]. apply ISM_Seq_Skip.
  - destruct H0. destruct H1 as [H2 H3]. split; [|now destruct b'; subst]. constructor; [|tauto].
  ...
  - split.
    ...
    + invert Hequnused_p; clear H1. unfold unused_prog in H. 
      rewrite Forall_forall in H. apply H with (x:=c). apply nth_error_In in e1; auto.
  - split.
...
Proof.
  intros. induction X0; [constructor|].
  eapply ideal_unused_overwrite_one_step in i; [|eassumption|eassumption]. destruct i.
...
Proof.
  ...
  eapply ideal_unused_overwrite with (X:=X) (n:=(st X)) in X0; [ |assumption|assumption].
  now rewrite !t_update_shadow in X0.
Qed.
...
Lemma spec_unused_overwrite_one_step (p:prog) : forall st ast b ds c c' st' ast' b' os X n,
  ...
  unused X c ->
  p |- <((c, st, ast, b))> -->_ds^^os <((c', st', ast', b'))> ->
  (Datatypes.prod (p |- <((c, X !-> n; st, ast, b))> -->_ds^^os <((c', X !-> n; st', ast', b'))>)
                  (unused X c')).
Proof.
  intros. remember (unused_prog X p) as unused_p. induction X0.
  - invert H0. repeat econstructor. rewrite t_update_permute; [constructor|assumption].
...
Proof.
  intros. induction X0; [constructor|].
  eapply spec_unused_overwrite_one_step in s; [|eassumption|eassumption]. destruct s.
  econstructor; [eassumption|tauto].
Qed.
...
Proof.
  intros. rewrite <- (t_update_same _ st X) at 1.
  eapply spec_unused_overwrite with (X:=X) (n:=(st X)) in X0; [|assumption|assumption].
  now rewrite !t_update_shadow in X0.
Qed.
...
Proof.
  ...
  intros. generalize dependent a. induction X; eauto; subst.
  apply IHX. clear IHX X.
  induction s; eauto; subst.
  2:clear g a. 1:rename a into a'.
...
Definition strong_induction_le_Type :
  forall A : nat -> Type,
  A 0 ->
  (forall n : nat, (forall m : nat, m <= n -> A m) -> A (S n)) ->
  forall n : nat, A n.
Proof.
  intros. assert (Le : forall n : nat, n <= n). { apply le_n. }
  enough (P : forall m : nat, m <= n -> A m); [firstorder|].
  clear Le. induction n; firstorder.
  - rewrite le_0_r in H; subst; assumption.
  - apply le_lt_eq_dec in H. destruct H.
    + assert (m <= n) by lia. apply IHn; assumption.
...
Lemma app_nil_r_tr : forall (A : Type) (l : list A), l ++ [] = l.
Proof.
  intros. induction l; auto. simpl; rewrite IHl; auto.
Defined.

Transparent app_nil_r_tr.
...
Proof.
  ...
  rewrite <- app_nil_r_tr with (l:=dirs2). rewrite <- app_nil_r_tr with (l:=obs2).
...
Lemma ultimate_slh_prog_contents:
  ...
  {c':_ & (cmd = (<{{("b" := ("callee" = (aeval st (n + e))) ? "b" : 1); (ultimate_slh c') }}>))}.
Proof.
  induction p. 
...
(* these broke but I'm still able to inline their tactics and it works ???

   at least with expose_src_cmd... *)
...
Ltac expose_src_cmd st H := specialize (ultimate_slh_prog_contents _ _ _ _ st H);
...
Proof.
  ...
  (* execution length of 0 *)
  { destruct tgt_exec; simpl in Heqn; [|lia]. do 2 rewrite t_update_same. eexists. split; try constructor; auto.
    subst; destruct c; try discriminate; auto.
  }
    ...
    - (* optimization *) inv s. inv X0. destruct tgt_exec.
      ...
      + simpl in Heqn. injection Heqn; intros. inv s.
        ...
        * rewrite H. simpl. reflexivity.
        ...
        simpl in *. inv s; try (inv X0). destruct tgt_exec. 
        * simpl in *. eexists. split; [|intros; discriminate]. rewrite t_update_permute; [|discriminate].
          rewrite t_update_shadow. do 2 rewrite t_update_same. econstructor.
        * simpl in *. inv s.
          -- (* DStep *) rewrite aeval_unused_update in H1; auto. specialize (ultimate_slh_prog_contents _ _ _ _ st H1). intros. 
             destruct H as (c_src & H). subst. eapply uslh_prog_to_uslh_com' in H1. simpl in *. 
             rewrite aeval_unused_update; auto. injection Heqn; intros. destruct tgt_exec.
             ++ simpl in *. rewrite t_update_permute; [|discriminate]. rewrite t_update_shadow.
                do 2 rewrite t_update_same. exists c_src. split.
                ** rewrite <- app_nil_r. rewrite <- app_nil_r with (l:=[DStep]).
                   econstructor; try econstructor.
                   { rewrite Hf. simpl. auto. }
                   { assumption. }
                ** intros; discriminate.
            ++ eapply X; eauto.  simpl in H.





             eapply X with (uslh_c:=(<{{ "b" := ("callee" = (aeval st f)) ? "b" : 1; (ultimate_slh c_src) }}>)) 
              in ast_arrs; eauto; cycle 1.
              ++ simpl. rewrite Hf.
             ++ rewrite aeval_unused_update; auto. rewrite t_update_eq in ast_arrs. rewrite t_update_neq in ast_arrs; [|discriminate].
                rewrite t_update_permute in ast_arrs; [|discriminate]. destruct ast_arrs as (c''0 & ast_arrs).
                exists c''0. 
                (* 
      spec_unused_overwrite
     : forall (p : prog) (st : state) (ast : astate) 
         (b : bool) (ds : dirs) (c c' : com) (st' : state) 
         (ast' : astate) (b' : bool) (os : obs) (X : string) 
         (n : nat),
       unused X c ->
       unused_prog X p ->
       p |- <(( c, st, ast, b ))> -->*_ ds ^^ os <(( c', st', ast', b' ))> ->
       p |- <(( c, X !-> n; st, ast, b ))> -->*_ ds ^^ os <(( c',
       X !-> n; st', ast', b' ))>
                *)
      ...
      subst. inv s. inv X0. simpl.
      specialize X with (uslh_c:=(<{{"b" := ("callee" = f)? "b" : 1; ultimate_slh c'}}>)).
      eapply X; eauto.



        - (* true *) simpl in Heqn. injection Heqn; intros. inv s. rewrite Hf in *. inv X0.
          
          
        inv X0. inv X2. inv X1; destruct (is_empty (vars_aexp f)) eqn:Hf; subst;
        try (clean_goal st_b; eexists; split; [econstructor|intros; discriminate]).
        - (* optimization *) inv X0; [inv X1|]. inv X2.
          + clean_goal st_b. eexists. split; [econstructor|intros; discriminate].
          + inv X0.
            * (* Step *) simpl. rewrite aeval_unused_update in H1 |- *; auto. 
              specialize (ultimate_slh_prog_contents _ _ _ _ st H1). intros. 
              destruct H as (c_src & H). subst. eapply uslh_prog_to_uslh_com' in H1.
              
              eapply X; eauto. simpl in X1. unfold exec_len in Heqn. simpl in Heqn. 
              simpl in tgt_exec. 
              
              admit.
            * (* Force *) rewrite aeval_unused_update in H1; auto.
              specialize (ultimate_slh_prog_contents _ _ _ _ st H2). intros.
              destruct H as (c_src & H). subst. eapply uslh_prog_to_uslh_com' in H2.
              simpl. eapply X; eauto. admit.
        - (* no optimization *) inv X0; [inv X1|]. 
          destruct b'1 eqn:Hmsf. Opaque unused.
          + (* already misspeculating *) destruct p; simpl in *.
            * (* empty program *) inv X2.
              -- clean_goal st_b. eexists. split; [|intros; discriminate].
                 rewrite <- app_nil_r. setoid_rewrite <- app_nil_r at 2.
                econstructor.
              -- inv X0; [|rewrite nth_error_nil in H2; discriminate].
                simpl in H1. rewrite t_update_neq, st_b, eqb_refl in H1; 
                [|discriminate]. discriminate.
            * (* nonempty program *) inv X2.
              ...
              -- inv X0.
                 ++ (* Step *)
                    rewrite st_b in *. rewrite eqb_refl in *. simpl in *.
                    rewrite t_update_neq in *; try discriminate. rewrite st_b in *.
                    rewrite eqb_refl in *. rewrite <- st_b. 
                    specialize (ultimate_slh_prog_contents _ _ _ _ st H1).
                    intros. destruct H as (c_src & H). subst. 
                    eapply uslh_prog_to_uslh_com' in H1. simpl in H1, X1.
                    change (DStep :: ds2) with ([DStep] ++ ds2). 
                    change (OCall 0 :: os2) with ([OCall 0] ++ os2).
                    inv X1.
                    ** simpl. rewrite t_update_permute; [|discriminate].
                      rewrite t_update_shadow. rewrite t_update_permute; [|discriminate].
                      exists c_src. split; [|intros; discriminate].
                      rewrite <- app_nil_r. rewrite <- app_nil_r with (l:=[DStep]).
                      do 2 rewrite t_update_same. econstructor; try econstructor; 
                      try rewrite Hf; eauto.
                    ** inv X0. inv X1. simpl. inv X2.
                      --- rewrite t_update_eq. simpl. rewrite t_update_shadow. 
                          rewrite t_update_permute; [|discriminate]. 
                          rewrite t_update_shadow. rewrite t_update_permute; [|discriminate].
                          exists c_src. split; [|intros; discriminate].
                          rewrite <- app_nil_r. rewrite <- app_nil_r with (l:=[DStep]).
                          do 2 rewrite t_update_same. econstructor; try econstructor; 
                          try rewrite Hf; eauto.
                      --- inv X0; [inv X2|]. simpl in X1 |- *. 
                          rewrite t_update_neq in X1; [|discriminate].
                          replace (if b' then st "b" else 0) with (if b' then 1 else 0); cycle 1.
                          { rewrite st_b. auto. } eapply X with (p:=(c::p)) (c:=<{{call f}}>) (ast:=ast'0)
                          (b:=true) (ds:=(DStep :: ds2)) (os:=(OCall 0 :: os2)); eauto.
                          admit.
                 ++ (* Force *)
                     simpl in H1. rewrite st_b in H1. rewrite eqb_refl in H1. 
                    rewrite t_update_neq in H1; [|discriminate]. rewrite st_b in H1. 
                    rewrite eqb_refl in H1. rewrite st_b in X1. rewrite eqb_refl in X1.
                    specialize (ultimate_slh_prog_contents _ _ _ _ st H2). intros. 
                    destruct H as (c_src & H). subst. eapply uslh_prog_to_uslh_com' in H2.
                    simpl in X1. eapply X with (p:=(c::p)) (c:=<{{call f}}>) (ast:=ast'0)
                    (b:=true) (ds:=([DForceCall j] ++ ds2)) (os:=([OForceCall] ++ os2)); eauto.
                    admit.
          + (* not yet misspeculating *) destruct p; simpl in *.
            * (* empty program *) inv X2.
              -- rewrite t_update_permute; [|discriminate]. rewrite t_update_shadow.
                ...
                exists <{{call f}}>. split; [|intros; discriminate]. 
                rewrite <- app_nil_r. setoid_rewrite <- app_nil_r at 2.
                econstructor.
              -- inv X0; [rename H1 into H2|]; unfold ultimate_slh_prog, 
                ultimate_slh_prog_gen in H2; rewrite nth_error_nil in H2; 
                discriminate.
            * (* nonempty program *) rewrite st_b in X2. simpl in X2. 
              eapply X with (p:=(c::p)) (c:=<{{call f}}>) (ds:=ds0) (os:=os0); eauto.
              admit.
      }
   - (* Asgn *) inv X0. inv X1; [|inv X0]. simpl. eexists. split;
     [|split; auto; rewrite t_update_neq; firstorder].
     rewrite t_update_permute with (x1:="b") (x2:=x);
     try (rewrite t_update_permute with (x1:="callee") (x2:=x));
     try firstorder. do 2 rewrite t_update_same. rewrite <- app_nil_r.
     rewrite <- app_nil_r with (l:=[]). econstructor; econstructor; eauto.
   - (* Seq *) inv X0.
     + eapply multi_spec_seq in X1. destruct X1.
       * do 7 destruct s. do 3 destruct p0. subst. eapply multi_spec_trans in X2; [|eapply m]. clear m.
         eapply X; eauto. admit.
       * destruct s, p0. subst. eapply multi_spec_trans in X2; [|apply m]. eapply X; eauto. admit.
     + simpl. eapply X; eauto. admit.
  ...
  (* - (* Seq-Skip *) What happened, did I accidentally solve this one without noticing? 
  ...
  - (* If *)
    ...
    +  simpl in X0. rewrite Hbe in X0. 
      destruct (beval st'0 be) eqn:Heq; inv X0; rewrite Heq in *; simpl in *.
      (* beval st'0 be = true *)
      * (* Step *) inv X1.
        -- do 2 rewrite t_update_same. eexists. split; [|intros; discriminate].
          rewrite <- app_nil_r. rewrite <- app_nil_r with (l:=[DStep]). econstructor; econstructor; firstorder.
          rewrite Heq, Hbe; auto.
        -- inv X0. inv X1. inv X2.
          ++ eexists. split; [|intros; discriminate]. simpl. rewrite t_update_shadow. 
              do 2 rewrite t_update_same. rewrite <- app_nil_r. rewrite <- app_nil_r with (l:=[DStep]).
              econstructor; econstructor; eauto. rewrite Hbe, Heq; firstorder.
          ++ inv X0; [inv X2|]. simpl. eapply X; eauto. admit.
      * (* Force *) eapply X; eauto. admit.
      (* beval st'0 be = false *)
      * (* Step *) inv X1.
        -- eexists. split; [|intros; discriminate]. do 2 rewrite t_update_same.
           rewrite <- app_nil_r. rewrite <- app_nil_r with (l:=[DStep]).
           econstructor; econstructor; eauto. rewrite Heq, Hbe; firstorder.
        -- inv X0. inv X1. eapply X; eauto. admit.
      * (* Force *) inv X1.
        -- eexists. split; [|intros; discriminate]. rewrite <- app_nil_r.
           rewrite <- app_nil_r with (l:=[DForce]). econstructor.
          ++ econstructor; eauto; rewrite Heq, Hbe; firstorder.
          ++ do 2 rewrite t_update_same. econstructor.
        -- inv X0. inv X1. inv X2.
          ++ simpl. rewrite Heq. rewrite t_update_shadow. 
    ...
    + case (beval st'0 be) eqn:Heq.
      * simpl in H0; destruct b; rewrite st_b in H0; simpl in H0.
        ...
        -- rewrite Heq in H0. invert H0; [solve_refl|]. invert H. 
           ...
           rewrite t_update_neq in st'0_st'; [|discriminate].
           erewrite <- t_update_same with (m:=st'0) in st'0_st' at 1. 
           erewrite t_update_permute in st'0_st'; [|discriminate].
