...
Reserved Notation
  ...
  (at level 40, pc custom com at level 99, pc' custom com at level 99,
   r constr, m constr, sk constr, r' constr, m' constr, sk' constr at next level).
...
So, a seq small step is SS inst pc r m sk pc' r' m' sk' os   
...
Inductive seq_eval_small_step_inst (p:prog) : 
  inst -> cptr -> reg -> mem -> (list cptr) -> 
  cptr -> reg -> mem -> (list cptr) -> obs -> Prop :=
  | SSMI_Skip : forall pc r m sk,
      p[pc] = <{{ skip }}> -> 
      p |- <((pc, r, m, sk))> -->^[] <((pc+1, r, m, sk))>
  | SSMI_Asgn : forall pc r m sk e x r',
      p[pc] = <{{ x := e }}> ->
      r' = r[x <- (eval r e)] ->
      p |- <((pc, r, m, sk))> -->^[] <((pc+1, r', m, sk))>
  | SSMI_Branch : forall e l n b pc r m sk pc',
      p[pc] = <{{ branch e l }}> -> 
      n = eval r e ->
      b = not_zero n -> 
      pc' = (match b with 
  ...
  | SSMI_Jump : forall l pc r m sk,
      p[pc] = <{{ jump l }}> -> 
      p |- <((pc, r, m, sk))> -->[] <(((l,0), r, m, sk))>
  | SSMI_Load : forall e x n pc r m sk r',
      p[pc] = <{{ x <- load[e] }}> ->
      n = eval r e ->
      r' = r[x <- m[n]] -> 
      p |- <((pc, r, m, sk))> -->^[OLoad n] <((pc+1, r', m, sk))>
  | SSMI_Store : forall e e' n v pc r m sk m',
      p[pc] = <{{ store[e] <- e' }}> ->
      n = eval r e ->
      v = eval r e' -> 
      m' = m[n <- v] ->
      p |- <((pc, r, m, sk))> -->^[OStore n] <((pc+1, r', m, sk))>
  | SSMI_Call : forall e l pc r m sk,
      p[pc] = <{{ call e }}> ->
      l = eval r e ->
      p |- <((pc, r, m, sk))> -->^[OCall l] <(((l,0), r, m, (pc+1::sk)))>
  ...
  where "p |- <(( pc , r , m , sk ))> -->^ os <(( pc' , r' , m' , sk' ))>" := 
      (seq_eval_small_step_inst p i pc r m sk pc' r' m' sk' os).
...
Proof.
  ...
  7 : { (* Call *) rename p0 into f. inv H1.
        inv H12. inv H8. 
        { rewrite t_update_permute; [|discriminate]. rewrite t_update_shadow.
          do 2 rewrite t_update_same. eexists. split; [econstructor|intros; discriminate].
        }
        inv H; [inv H12|].
        destruct (is_empty (vars_aexp f)) eqn:Hf.
        { (* optimization *) inv H0; try refl H. inv H.
          { (* DStep *) rewrite aeval_unused_update in *; auto. inv H1.
            { clean_goal st_b. simpl. expose_src_cmd st H3. exists c_src.
              split; [|intros; inv H]. eapply multi_ideal_trans_nil_r;
              [constructor; [rewrite Hf|]; eauto|econstructor].
            }
            expose_src_cmd st H3. inv H. inv H13. inv H0.
            - clean_goal st_b. exists c_src. split; [eapply multi_ideal_trans_nil_r; econstructor;
              [rewrite Hf|]; eauto|intros; discriminate].
            - inv H; [inv H13|]. simpl in H1 |- *. rewrite t_update_eq in H1. rewrite eqb_refl in H1.
              rewrite t_update_same in H1. eapply IH in H1; try (eapply unused_vars); eauto.
              rewrite t_update_neq in H1; [|discriminate]. rewrite t_update_eq in H1. 
              destruct H1 as (c_src' & H1). inv H1. exists c_src'.
              split; auto. fold_cons.
              eapply ideal_unused_update in H; try (eapply unused_vars); eauto.
              econstructor; [constructor; [rewrite Hf|]; eauto|eauto].
          }
          { (* DForceCall *) inv H1.
            { expose_src_cmd st H4. clean_goal st_b. rewrite aeval_unused_update in *; auto.
              exists c_src. simpl. split; [|discriminate]. eapply multi_ideal_trans_nil_r;
              econstructor; try (rewrite Hf); eauto.
            }
            { rewrite aeval_unused_update in *; auto. simpl. expose_src_cmd st H4. inv H.
              inv H14. inv H0.
              { simpl. clean_goal st_b. exists c_src. split; [|discriminate]. 
              ...
              rewrite eqb_neq in H3. simpl in H0 |- *. rewrite H0 in H.
              inv H; [inv H15|].
              apply IH in H1; try (eapply unused_vars);
              try (rewrite H1; rewrite t_update_eq); eauto.
              rewrite t_update_eq in H1. rewrite t_update_neq in H1; [|discriminate].
              rewrite t_update_eq in H1.
              destruct H1 as (c_src' & H1). inv H1.
              exists c_src'. split; auto. fold_cons. 
        ...
        { (* no optimization *) inv H0.
          { clean_goal st_b. simpl. eexists. split; [|intros; discriminate]. econstructor. }
          inv H; simpl.
          { (* DStep *) simpl in H1, H3, IH. destruct b'0 eqn:Hmsf; rewrite st_b in *.
            { (* already misspeculating *) rewrite eqb_refl in *. rewrite t_update_neq in *; try discriminate.
              rewrite st_b, eqb_refl in H3. destruct p; simpl in *; try discriminate. rewrite st_b. simpl.
              (* p is nonempty *) inv H3. destruct H1.
              ...
              { inv H. inv H12. inv H1.
                ...
                - inv H; [inv H12|]. apply IH in H0; try (eapply unused_vars_cons); eauto. 
                  simpl in H0. rewrite t_update_eq in H0. do 2 (rewrite t_update_neq in H0; [|discriminate]).
                  rewrite t_update_eq in H0. rewrite t_update_permute in H0; [|discriminate]. 
                  destruct H0 as (c_src' & H0). inv H0. simpl.
                  exists c_src'. split; auto. fold_cons. econstructor; [constructor; [rewrite Hf|simpl]; eauto|].
                  rewrite <- st_b. rewrite t_update_same in H. apply ideal_unused_update in H;
                  try (eapply unused_vars_cons); eauto.
              }
            }
            { (* not yet misspeculating *) simpl in H1, H3, IH |- *. rewrite aeval_unused_update in H3; auto.
              ...
              (* p is nonempty *) inv H3. inv H1.
              { simpl. clean_goal st_b. rewrite st_b. rewrite aeval_unused_update; auto.
                expose_src_cmd st H0. exists c_src. split; [|split; [discriminate|auto] ].
              ...
              { rewrite t_update_neq; [|discriminate]. rewrite aeval_unused_update; [|apply unused_c_callee].
                rewrite st_b. simpl. fold_cons. expose_src_cmd st H0. inv H. inv H13. inv H2.
                - clean_goal st_b. exists c_src. split; [|intros; discriminate].
                  clean_ds_os [DStep]. econstructor; econstructor; [rewrite Hf|]; auto.
                - inv H; [inv H13|]. apply IH in H1; try (eapply unused_vars); eauto;
                  [|simpl; do 2 rewrite t_update_eq; rewrite Nat.eqb_refl; rewrite t_update_neq; [|discriminate]; auto].
                  rewrite t_update_eq in H1. rewrite t_update_neq in H1; [|discriminate].
                  rewrite t_update_eq in H1. simpl in H1. rewrite t_update_eq, eqb_refl in H1.
                  rewrite t_update_neq in H1; [|discriminate]. rewrite t_update_permute in H1; [|discriminate].
                  destruct H1 as (c_src' & H1). inv H1. exists c_src'. split; auto.
                  econstructor; [econstructor; [rewrite Hf|]; eauto|].
          ...
          { (* DForceCall *) simpl in H1, H3, IH. destruct b'1 eqn:Hmsf; rewrite st_b in *.
            { (* already misspeculating *) rewrite t_update_neq in H3; [|discriminate]. rewrite st_b in H3.
              rewrite Nat.eqb_refl in H1, H3. destruct p; simpl in *; [rewrite nth_error_nil in H4; discriminate|].
              (* p is nonempty *) inv H4. inv H1.
              - clean_goal st_b. expose_src_cmd st H0. exists c_src. split; [|split; [discriminate|auto] ].
                eapply multi_ideal_trans_nil_r; [econstructor; [rewrite Hf|eapply H3|]; eauto|econstructor].
              - expose_src_cmd st H0. inv H. inv H14. inv H2.
                ...
                + inv H; [inv H14|]. apply IH in H1; try (eapply unused_vars); eauto; 
                  [|rewrite t_update_eq; simpl; try (induction j); auto]. simpl in H1. 
                  rewrite j_not_zero in H1; auto. rewrite t_update_eq in H1. rewrite t_update_neq in H1; 
                  [|discriminate]. rewrite t_update_eq in H1. rewrite t_update_permute in H1; [|discriminate].
                  rewrite <- st_b in H1. rewrite t_update_same in H1.
                  destruct H1 as (c_src' & H1). destruct H1 as (H1 & rest).
                  exists c_src'. split; [|rewrite <- st_b; eauto]. fold_cons.
                  econstructor; [econstructor; [rewrite Hf|eapply H3|]; eauto|rewrite <- st_b].
                  apply ideal_unused_update in H1; try (eapply unused_vars); eauto.
            }
            { (* not yet misspeculating *) simpl in H3. rewrite aeval_unused_update in H3; auto.
              rewrite t_update_neq in H3; [|discriminate]. rewrite st_b in H3. simpl in H3.
              destruct p; [rewrite nth_error_nil in H4; discriminate|].
              (* p is nonempty *) inv H4. inv H1.
              - clean_goal st_b. expose_src_cmd st H0. exists c_src. split; try split; try discriminate.
              ...
              - expose_src_cmd st H0. inv H. inv H14. inv H2.
                ...
                + inv H; [inv H14|].
                  simpl in H1. rewrite t_update_eq in H1. rewrite t_update_neq in H1; [|discriminate].
                  rewrite <- eqb_neq in H3. rewrite H3 in H1.
                  apply IH in H1; try (eapply unused_vars); eauto.
                  rewrite t_update_eq in H1. rewrite t_update_neq in H1; [|discriminate].
                  rewrite t_update_eq in H1. rewrite t_update_permute in H1; [|discriminate].
                  destruct H1 as (c_src' & H1). inv H1. exists c_src'. split; [|auto]. fold_cons.
  ...
  - (* Skip *) inv H1. 
  - (* Asgn *) inv H1. invert H8; [|now inversion H]. eexists. split; 
  ...
  - (* If *) inv H1; destruct (is_empty (vars_bexp be)) eqn:Hbe.
    + simpl in H8. destruct (beval st'0 be) eqn:Heq.
      * invert H8; [solve_refl|].
        invert H. invert H12. invert H0; [solve_refl|].
        ...
        2 : { inversion unused_c. destruct H0. auto. }
        2 : { inversion unused_c_callee. destruct H0. auto. }
        destruct H1 as (c''&st'0_st'&H').
    ...
    + case (beval st'0 be) eqn:Heq.
      * simpl in H8; destruct b'0; rewrite st_b in H8; simpl in H8.
        ++ invert H8; [solve_refl|]. invert H. invert H12. invert H0; [solve_refl|].
           invert H; [inversion H12|]. simpl in H1. rewrite st_b in H1; simpl in H1. rewrite <- st_b, t_update_same in H1.
           eapply IH in H1; eauto.
           2 : { inversion unused_c. destruct H0. auto. }
           2 : { inversion unused_c_callee. destruct H0. auto. }
           destruct H1 as (c''&st'0_st'&H'). eexists. simpl. rewrite st_b at 2. simpl.
           split; [|eassumption]. now com_step.
        ++ rewrite Heq in H8. invert H8; [solve_refl|]. invert H. invert H12. invert H0; [solve_refl|]. 
           invert H; [inversion H12|].
           simpl in H1. rewrite st_b, Heq in H1. simpl in H1. rewrite <- st_b, t_update_same in H1.
           apply IH in H1; auto.
           2 : { inversion unused_c. destruct H0. auto. }
      ...
      * simpl in H8. rewrite Heq, andb_false_r in H8. invert H8; [solve_refl|]. invert H. invert H12. 
        invert H0; [solve_refl|].
        invert H; [inversion H12|]. simpl in H1. rewrite Heq, andb_false_r in H1. rewrite t_update_same in H1.
        apply IH in H1; auto.
        2 : { inversion unused_c. destruct H0. auto. }
        2 : { inversion unused_c_callee. destruct H0. auto. }
        destruct H1 as (c''&st'0_st'&H'). eexists. simpl. rewrite Heq, andb_false_r. simpl.
        split; [|eassumption]. now com_step.
    + simpl in H8. destruct (beval st'0 be) eqn:Heq.
      * invert H8; [solve_refl|].
        invert H. invert H12. invert H0; [solve_refl|].
        invert H; [inversion H12|].
        simpl in H1. rewrite st_b, Heq in H1. simpl in H1.
        eapply IH in H1; eauto.
        2 : { inversion unused_c. destruct H0. auto. }
        2 : { inversion unused_c_callee. destruct H0. auto. }
        destruct H1 as (c''&st'0_st'&H'). rewrite t_update_eq in st'0_st'. rewrite t_update_neq in st'0_st'; [|discriminate].
      ...
      * invert H8; [solve_refl|].
        ...
        -- discriminate.
    + case (beval st'0 be) eqn:Heq.
      * simpl in H8; destruct b; rewrite st_b in H8; simpl in H8.
        -- invert H8; [solve_refl|]. invert H. invert H12. invert H0; [solve_refl|].
           invert H; [inversion H12|]. simpl in H1. rewrite st_b in H1; simpl in H1. rewrite <- st_b, t_update_same in H1.
           apply IH in H1; auto.
           2 : { inversion unused_c. destruct H0. auto. }
           ...
           destruct H1 as (c''&st'0_st'&H'). eexists. simpl. rewrite st_b at 2. simpl.
           split; [|eassumption]. now com_step.
        -- rewrite Heq in H8. invert H8; [solve_refl|]. invert H. 
           invert H12. invert H0; [solve_refl|]. 
           invert H; [inversion H12|].
           simpl in H1. rewrite st_b, Heq in H1. simpl in H1.
           apply IH in H1; auto.
           2 : { inversion unused_c. destruct H0. auto. }
           2 : { inversion unused_c_callee. destruct H0. auto. }
           destruct H1 as (c''&st'0_st'&H'). exists c''. simpl. 
      ...
      * simpl in H8. rewrite Heq, andb_false_r in H8. invert H8; [solve_refl|]. invert H. invert H12. 
        invert H0; [solve_refl|].
        invert H; [inversion H12|]. simpl in H1. rewrite Heq, andb_false_r in H1.
        apply IH in H1; auto.
        2 : { inversion unused_c. destruct H0. auto. }
        2 : { inversion unused_c_callee. destruct H0. auto. }
        destruct H1 as (c''&st'0_st'&H'). exists c''. simpl. rewrite Heq, andb_false_r. simpl.
  ...
  - (* While *) inv H1.
    ...
    + destruct (is_empty (vars_bexp be)) eqn:Hbe.
      ...
      * destruct (beval st'1 be) eqn:Heq.
        (* be true *)
        -- invert H0; [solve_refl|]. invert H. invert H12. invert H11. invert H12. invert H1; 
           [solve_refl|].
           invert H. invert H12. invert H11; [inversion H12|]. 
           apply multi_spec_seq_assoc in H0.
           destruct H0 as (?&H&H'). apply multi_spec_seq in H. destruct H.
           ++ do 10 destruct H. destruct H0, H1, H2. subst. simpl in H2. 
              rewrite Heq, t_update_same in H2.
              apply IH in H2; try tauto; auto; try lia.
        ...
        (* be false *)
        -- invert H0; [solve_refl|]. invert H; [inversion H12|]. invert H1; [solve_refl|]. invert H. 
      ...
      * destruct (beval st'1 be) eqn:Heq.
        ...
        -- destruct b'1. 
           (* b'1 true *)
           ++ simpl in H0. rewrite st_b, Heq in H0. simpl in H0. invert H0; [solve_refl|].
              invert H; [inversion H12|]. invert H1; [solve_refl|]. invert H. invert H0; [|inversion H].
           ...
           (* b'1 false *)
           ++ simpl in H0. rewrite st_b, Heq in H0. simpl in H0. apply multi_spec_seq_assoc in H0.
              destruct H0 as (?&H&H'). apply multi_spec_seq in H. destruct H.
              ** do 10 destruct H. destruct H0, H1, H2. subst. simpl in H2. invert H2. invert H. invert H13. 
                 invert H0. invert H; [inversion H13|].
                 simpl in H1. rewrite st_b, Heq in H1. simpl in H1. rewrite <- st_b, t_update_same in H1.
                 apply IH in H1; auto; try tauto; try lia.
        ...
        (* be false *)
        -- simpl in H0. rewrite Heq, andb_false_r in H0. invert H0; [solve_refl|]. invert H; [inversion H12|]. 
           invert H1; [solve_refl|]. invert H. invert H0; [|inversion H].
    ...
    + destruct (is_empty (vars_bexp be)) eqn:Hbe.
      ...
      * destruct (beval st'1 be) eqn:Heq.
        ...
        (* be false *)
        -- invert H0; [solve_refl|]. invert H. invert H12. invert H11. invert H12. invert H1; [solve_refl|].
           invert H. invert H12. invert H11; [inversion H12|]. apply multi_spec_seq_assoc in H0.
           destruct H0 as (?&H&H'). apply multi_spec_seq in H. destruct H.
           ++ do 10 destruct H. destruct H0, H1, H2. subst. simpl in H2. rewrite Heq in H2.
              ...
              replace <{{while be do "b" := be ? "b" : 1; 
                ...
                rewrite Hbe.
              pose proof (ideal_eval_preserves_nonempty_arrs _ _ _ _ _ _ _ _ _ _ _ ast_arrs H).
              apply IH in H3; auto; try lia. destruct H3 as (c''&H1&H1'').
              eexists. split; [|now intro c'_skip; apply H' in c'_skip; apply H1'']. 
              com_step. simpl.
      ...
      * destruct (beval st'1 be) eqn:Heq.
        (* be true *)
        -- simpl in H0. rewrite st_b, Heq in H0. destruct b'0.
           ++ simpl in H0. invert H0; [solve_refl|]. invert H. invert H12. invert H11. invert H12. 
              invert H1; [solve_refl|]. invert H. invert H12.
              invert H11; [inversion H12|]. apply multi_spec_seq_assoc in H0.
              destruct H0 as (?&H&H'). apply multi_spec_seq in H. destruct H.
              ** do 10 destruct H. destruct H0, H1, H2. subst. simpl in H2. rewrite st_b, Heq in H2. simpl in H2. 
                 rewrite <- st_b, t_update_same in H2.
                 apply IH in H2; auto; try tauto; try lia.
              ...
              ** do 2 destruct H. subst. simpl in H0. rewrite st_b, Heq in H0. simpl in H0. 
                 rewrite <- st_b, t_update_same in H0.
                 apply IH in H0; auto; try tauto; try lia. 
           ...
           ++ simpl in H0. invert H0; [solve_refl|]. invert H; [inversion H12|]. invert H1; [solve_refl|]. invert H. invert H0; [|inversion H].
        ...
        (* be false *)
        -- simpl in H0. rewrite Heq, andb_false_r in H0.
           invert H0; [solve_refl|]. invert H. invert H12. invert H11. invert H12. 
           invert H1; [solve_refl|]. invert H. invert H12.
           invert H11; [inversion H12|]. apply multi_spec_seq_assoc in H0.
           destruct H0 as (c''&H&H'). apply multi_spec_seq in H. destruct H.
           ++ destruct H as (st'0&ast'0&b''0&ds1&ds2&os1&os2&H). do 2 destruct H.
              ...
              apply IH in H; try tauto; try lia.
              ...
              pose proof (ideal_eval_preserves_nonempty_arrs _ _ _ _ _ _ _ _ _ _ _ ast_arrs Hc). 
              apply IH in H0; auto; try lia. destruct H0. destruct H0.
  ...
  - inv H1.
    + (* Read *) invert H8; [|inversion H].
      ...
      * eexists. split; try split; [|auto|rewrite t_update_neq; inv unused_c; auto].
      ...
      rewrite t_update_same. rewrite t_update_permute; [|inv unused_c_callee; auto].
      rewrite t_update_same. constructor.
    + (* Load *) invert H8; [|inversion H]. 
      destruct (is_empty (vars_aexp i)) eqn:Heq.
      * setoid_rewrite t_update_permute at 2; [|inv unused_c; auto].    
      rewrite t_update_same. rewrite t_update_permute; [|inv unused_c_callee; auto].
      rewrite t_update_same. eexists. split; try split; try rewrite t_update_neq; 
      firstorder; eauto. repeat econstructor; tauto.
      * simpl in H14. rewrite st_b in H14. simpl in H14.
      specialize (ast_arrs a). lia.
  - inv H1.
    + (* Write *) invert H8; [|inversion H].
      destruct (is_empty (vars_aexp i)) eqn:Heq.
      ...
      * eexists. split; [|tauto]. repeat econstructor; 
      ...
      do 2 rewrite t_update_same. constructor.
    + (* Store *) destruct (is_empty (vars_aexp i)) eqn:Heq.
      * invert H8; [|inversion H]. do 2 rewrite t_update_same.
