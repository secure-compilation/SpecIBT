...
Definition ct_to_tail : Type := list (list inst * bool).

Fixpoint group_by_proc_impl (p: prog) (current_proc_acc: ct_to_tail) : list ct_to_tail :=
  match p with
...
Definition group_by_proc (p: prog) : list ct_to_tail :=
...
Definition proc_map := (fun (x: ct_to_tail) => Datatypes.length x).

(* pst: list of # of blocks in each ct_to_tail *)
